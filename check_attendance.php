...
<div id="chk_clone" style="display: none">
	<div class="d-flex justify-content-center chk-opts">
		...
		<div class="form-check form-check-inline">
		  ...
		  <label class="form-check-label absent-lbl">Vắng phép</label>
		</div>
		<div class="form-check form-check-inline">
		  <input class="form-check-input late-inp" type="checkbox" value="2">
		  <label class="form-check-label late-lbl">Vắng không phép</label>
		</div>
	</div>
</div>
<style>
		...
		$.ajax({
			...
			success:function(resp){
				if(resp==1){
					  ...
					  alert_toast("Lớp này đã ghi nhận phiên điểm danh từ sinh viên.",'danger')
					  end_load();
				}
			}
		})
